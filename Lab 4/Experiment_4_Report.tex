...
\noindent \textbf{GitHub Repository:} \href{PASTE_YOUR_GITHUB_LINK_HERE}{https://github.com/Ankith-Davey/Deep-Learning-Lab-Ankith-24011101004/tree/main/Lab%204}
...
\item \textbf{Implement transfer learning using VGG16.} Done --
frozen-base transfer learning, 79.96\% test accuracy (Section 19.2).

\item \textbf{Repeat the experiment using ResNet50.} Done --
frozen-base transfer learning, 84.81\% test accuracy (Section 19.2).

\item \textbf{Compare Adam and SGD optimizers.} Done as part of the
Section 17 hyperparameter study -- Adam (baseline) got 85.67\%, SGD got
85.85\%, basically no real difference at this scale.

\item \textbf{Compare training with frozen layers and fine tuning.}
Done -- covered in both Task 4 and Section 17. Proper fine-tuning (8
epochs, low learning rate) moved MobileNetV2 from 85.49\% to 85.85\%, while
a rushed 5-epoch partial-unfreeze in the hyperparameter study actually
\emph{dropped} accuracy to 66.67\%. So unfreezing layers only helps if you
give it enough epochs and a low enough learning rate.

\item \textbf{Evaluate the model using Precision, Recall and F1-score.}
Done -- see Section 19.1 (MobileNetV2: Precision 86.05\%, Recall 85.10\%,
F1 85.02\%) plus the per-model classification reports in the notebook.

\item \textbf{Compare the performance of LeNet, AlexNet and ResNet.}
Done -- LeNet-5 (scratch) 50.65\%, AlexNet (scratch, scaled variant)
71.52\%, ResNet50 (transfer learning) 84.81\%. Part of that gap is
architecture, part of it is ResNet50 having access to pretrained ImageNet
features that the other two didn't.
...
Also worth being upfront about: the GoogleNet row in the results table isn't

...
98.01\% by the final (15th) epoch, while the validation accuracy peaked around
...
It won the ImageNet in 2012 by a huge margin and proved that deep CNNs
...
became the default approach for computer vision.
...
$3\times3$, $5\times5$, and pooling all in parallel and then concatenates the
results, so it captures features at different scales at once. The
$1\times1$ conv before the bigger ones cut down the parameter count a lot and that is the exact reason why GoogleNet has only 6.8M parameters while
VGG16 needs 138M.
...
LeNet-5 is small with 7 layers and just 60K parameters, built for small grayscale
digit images. On the other hand, ResNet50 is 50 layers and 25.6M parameters, built for
large-scale color images, and only works because of skip connections. In the experiment, LeNet-5 got 50.65\% from scratch,
ResNet50 got 84.81\% with transfer learning where both the extra depth and the
pretrained features contribute to the higher accuracy.

\item \textbf{What is Transfer Learning?}

Instead of training a network from random weights, training starts from one
already trained on a big dataset (usually ImageNet) and is reused. Most of
what it learned early on carries over to
...
backbone's features are still tuned for ImageNet, and not on the actual dataset.
Fine-tuning unfreezes some of the later layers and trains them a bit more
(at a low learning rate) so that they adapt to the new data. In the experiment, it moved
MobileNetV2 from 85.49\% to 85.85\% producing a small gain.
...
The models already know general visual features from millions of images, so
...
much more than LeNet-5 despite the huge parameter gap. This mostly because most
of ResNet50's weights were frozen and never touched by backpropagation. That's
...
reached 79.96\% test accuracy which was lower than ResNet50 and
...
transfer learning procedure was applied to ResNet50 and it achieved a test accuracy of 84.81\% which was noticeably ahead of VGG16 despite using
fewer parameters.
...
pretty well. LeNet-5 trained from scratch only got to 50.65\% given that it has under 100K parameters and never saw anything beyond and was trained only on CIFAR-10's own 50,000 training images. AlexNet (the scaled-down version used
here) did better at 71.52\%, trained from scratch. The three transfer-learning models tell a different story -- VGG16, ResNet50 and MobileNetV2 all started from ImageNet weights and landed at 79.96\%, 84.81\% and 85.10\% respectively, and got there in a fraction of the epochs the from-scratch models needed. MobileNetV2 was already past 85\% by epoch 2, which none of the models that were trained from scratch got close to at any point.
...
from 85.49\% to 85.85\%. The gain was small, but the hyperparameter study showed that doing the same kind of unfreezing carelessly (5 epochs, no
...
actually respecting how much the pretrained weights are let to shift.
